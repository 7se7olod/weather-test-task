...
import {
  List,
  Response5DaysForecastType,
} from '../types/response-five-days-forecast-weather.type';
...
export class WeatherService {
  ...
  getCityFromIp(): Observable<{ city: string }> {
    return this.http
      ...
      .pipe(
        switchMap((response) =>
          this.http.get<{ city: string }>(
            `https://ipapi.co/${response.ip}/json/`
          )
        ),
        tap((response) => {
  ...
  getWeather(
    ...
    return combineLatest([
      this.http.get<ResponseWeatherType>(environment.API_URL_CURRENT_WEATHER, {
        params,
      }),
      this.http.get<Response5DaysForecastType>(environment.API_URL_FIVE_DAYS, {
        params,
      }),
    ]).pipe(
      map(([weather, fiveWeather]) => {
        const res = {
          ...
          list: fiveWeather.list.filter(
            (weather: List) =>
              new Date(weather.dt * 1000).getHours() > 12 &&
              new Date(weather.dt * 1000).getHours() < 16
          ),
        };
        return {
          ...weather,
