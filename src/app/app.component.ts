...
export class AppComponent implements OnInit {
  ...
  styleColorTime = {backgroundCard: '#00c6ff', backgroundMain: 'white', fontColor: 'black', filter: 'none'};
  ...
  ngOnInit(): void {
    navigator.geolocation.getCurrentPosition((position: GeolocationPosition) => {
      if (position.coords) {
        this.weatherService
          .getWeatherTEST('', position.coords.longitude, position.coords.latitude)
          .pipe(
            tap((weather) => {
              this.styleColorTime = this.timeColorService.getStylesForTime(weather.timezone);
            }),
            switchMap(weather => this.chartService.getChartOptions(weather).pipe(
                tap(options => this.chartOptions$.next(options))
              )
            ),
            take(1),
          )
  ...
  public searchCityWeather(city: string): void {
    if (city) {
      this.weatherService.getWeatherTEST(city)
        .pipe(
          tap(weather => this.styleColorTime = this.timeColorService.getStylesForTime(weather.timezone)),
